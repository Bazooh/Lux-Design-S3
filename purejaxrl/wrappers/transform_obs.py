...
class HybridTransformObs(TransformObs):
    ...
    def __init__(self):
        ...
        self.vector_names = list(self.vector_features.keys())
        self.vector_size = sum(self.vector_features.values())
        self.n_image_channels = len(self.image_channel_names)

        self.observation_shape = (self.n_image_channels, 24, 24)
    
    @partial(jax.jit, static_argnums=(0, 1))
    def convert(
        self, team_id_str: str,
        obs: EnvObs, 
        params: EnvParams,
        reward: float | None = None,
    ):
        ...
        image = jax.numpy.zeros(
            (len(self.image_channel_names), obs.map_features.energy.shape[0], obs.map_features.energy.shape[1]),
        ...
        #return image #, vector
        return jax.numpy.arange(24, dtype = jax.numpy.float32)