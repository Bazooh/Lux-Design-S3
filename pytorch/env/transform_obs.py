...
class HybridTransformObs(TransformObs):
    ...
    def __init__(self):
        ...
        self.observation_space = gymnax.environments.spaces.Dict({
                            'image': gymnax.environments.spaces.Box(low=-1, high=1, shape=(self.image_channels, 24, 24), dtype=np.int8), 
                            'vector': gymnax.environments.spaces.Box(low=-1, high=1, shape=(self.vector_size), dtype=np.float32),
                            'time': gymnax.environments.spaces.Box(low=0, high=1, shape=(self.time_size), dtype=np.float32),
                            'position': gymnax.environments.spaces.Box(low=0, high=23, shape=(16, 2), dtype=np.int8), 
                            'mask_awake': gymnax.environments.spaces.Box(low=0, high=1, shape=(16), dtype=np.int8),
                            'action_mask': gymnax.environments.spaces.Box(low=0, high=1, shape=(16, 6), dtype=np.int8),
        })
