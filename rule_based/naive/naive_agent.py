...
class NaiveAgent(Agent):
    def actions(
        ...
        """
        actions = np.zeros((self.env_params.max_units, 3), dtype=np.int32)
        ...
        for unit_id in obs.get_available_units(self.team_id):
            ...
            if len(self.relic_node_positions) > 0:
                ...
                if (
                    ...
                    rand_loc = (
                        np.random.randint(0, self.env_params.map_width),
                        np.random.randint(0, self.env_params.map_height),
                    )
